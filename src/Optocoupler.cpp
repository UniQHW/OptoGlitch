...
Optocoupler::apply_set_cmd(String cmd)
{
  ...
  if (set_args.substring(0, sizeof(SET_ARG_CALIBRATION_MODE)) == APPEND_SPACE(SET_ARG_CALIBRATION_MODE))
  {
    ...
    // No calibration
    if (calibration_mode_str.substring(0, sizeof(CALIBARTION_MODE_ARG_NONE)) == CALIBARTION_MODE_ARG_NONE) {
      calibration_mode = none;
    }

    // Standard deviation
    else if (calibration_mode_str.substring(0, sizeof(CALIBARTION_MODE_ARG_STDDEV)) == CALIBARTION_MODE_ARG_STDDEV) {
      calibration_mode = stddev;
    }

    // Lookup table
    else if (calibration_mode_str.substring(0, sizeof(CALIBARTION_MODE_ARG_LOOKUP)) == CALIBARTION_MODE_ARG_LOOKUP) {
